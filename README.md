...
| 功能模块   | 描述                                                 | 状态     |
|--------|----------------------------------------------------|--------|
| 知识中心   | 包含知识分类和文件管理，助力用户迅速定位所需知识资源。                        | ✅ 已完成  |
| 概念配置   | 自定义配置知识抽取的概念规则。                                    | ✅ 已完成  |
| 关系配置   | 自定义配置知识抽取的关系规则。                                    | ✅ 已完成  |
| 非结构化抽取 | 运用预训练模型，从非结构化数据源中抽取有价值的知识。                         | ✅ 已完成  |
| 构化抽取   | 从数据库等结构化数据集中系统性地抽取、转换并加载知识元素。                      | ✅ 已完成  |
| 图谱探索   | 提供直观且交互性强的图形界面，可视化地浏览和分析复杂的知识图谱。                   | ✅ 已完成  |
| 系统管理   | 系统的基础信息管理，包括用户管理、角色管理、部门管理、菜单管理、日志管理等功能。           | ✅ 已完成  |
| 知识融合   | 整合来自不同源的知识，通过融合策略提高知识的一致性和准确性。                     | ❌ 未来计划 |
| 知识推理   | 使用逻辑推理技术增强知识的理解和应用，不仅能理解知识间的直接关联，还能推断出潜在的间接关系。     | ❌ 未来计划 |
| 知识问答   | 基于大模型技术（LLM），集成NLP技术和知识图谱的智能问答平台，允许通过自然语言提问。       | ❌ 未来计划 |
| 知识检索   | 基于知识库检索模型，结合全文检索技术和语义分析，允许通过自然语言对知识库内容的快速而准确的搜索功能。 | ❌ 未来计划 |
| 服务管理   | 一站式的后台服务管理解决方案，支持服务的注册、监控、维护及优化。                   | ❌ 未来计划 |
| 服务日志   | 自动记录所有服务请求和服务响应详情，便于进行故障排查和性能评估。                   | ❌ 未来计划 |
...
<table>
    ...
    <tr>
        <td><img alt="" src=".gitee/system/extUnstructTask.png"/></td>
        <td><img alt="" src=".gitee/system/extResult-2.png"/></td>
    </tr>
    <tr>
        <td><img alt="" src=".gitee/system/extResult-1.png"/></td>

                        ...
                        <el-table :data="form.tableData" style="width: 100%">
                            <el-table-column label="实体名称1" min-width="180">
                                <template #default="scope">
                                    <el-input
                                        v-model="scope.row.entityName1"
                                        readonly
                                        @click="openEntity(scope.row, 'pre')"
                                        type="text"
                                        placeholder="请选择实体名称"
                                    />
                                </template>
                            ...
                            <el-table-column label="关系名称" min-width="180">
                                <template #default="scope">
                                    <el-input
                                        v-model="scope.row.relationshipType"
                                        type="text"
                                        placeholder="请填写名称"
                                    />
                                </template>
                            ...
                            <el-table-column label="实体名称2" min-width="180">
                                <template #default="scope">
                                    <el-input
                                        v-model="scope.row.entityName2"
                                        readonly
                                        @click="openEntity(scope.row, 'post')"
                                        type="text"
                                        placeholder="请选择实体名称"
                                    />
                                </template>
...
<script setup name="addEntity">
    ...
    function submitForm() {
        proxy.$refs['handleRef'].validate(async (valid) => {
            if (valid) {
                if (
                    form.value.tableData[0].entityId1 == null ||
                    form.value.tableData[0].entityId1 == undefined
                ) {

        ...
        <div class="pagecont-bottom">
            ...
            >
                ...
                >
                    <template #default="scope">
                        <div>
                            <dict-tag :options="ext_update_type" :value="scope.row.updateType" />
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                >
                    <template #default="scope">
                        ...
                        <template v-if="scope.row.publishStatus != 1 && scope.row.status == 2">
                            <el-popover
                                placement="bottom"
                                :width="150"
                                trigger="click"
...
<script setup name="ExtStruct">
    ...
    const data = reactive({
        ...
        rules: {
            ...
            updateType: [{ required: true, message: '更新类型不能为空', trigger: 'blur' }],
            updateFrequency: [{ required: true, message: '更新频率不能为空', trigger: 'blur' }],
    ...
    const taskDescriptionMap = {
        用户信息抽取: '从用户资料中抽取姓名、昵称、年龄、邮箱、标签、注册时间等基础属性信息。',
        医疗信息抽取: '面向医疗业务数据抽取疾病、诊断、检查、治疗、药物等核心医学信息。',
        医疗器械信息抽取: '抽取医疗器械名称、型号、类别、适用科室、生产企业及基础管理信息。',
        疾病治疗信息抽取: '围绕疾病治疗过程抽取治疗方式、用药方案、干预措施和疗效相关信息。',
        人物信息综合抽取: '从人物资料中综合抽取身份信息、角色标签、所属关系及个人属性数据。',
        疾病治疗综合抽取: '整合疾病、症状、诊断、治疗方案和康复建议，形成疾病治疗综合信息。',
        企业信息综合抽取: '抽取企业名称、行业类型、注册地址、经营范围、联系方式等企业基础信息。',
        企业人员信息抽取: '抽取企业人员姓名、岗位、部门、职责、联系方式及组织归属信息。',
        药物治疗信息抽取: '抽取药物名称、适应症、用法用量、不良反应和治疗场景等用药信息。',
        医疗器械使用信息抽取: '抽取医疗器械使用流程、操作规范、适用场景、维护要求及安全注意事项。',
        健康人员体检抽取: '抽取体检人员基础信息、检查项目、体检指标、异常结果和健康建议。'
    };
    ...
    const getTaskDescription = (row) => {
        ...
        return taskDescriptionMap[taskName] || taskDescriptionIdMap[row.id] || row.remark || '-';
    };

...
<template>
    ...
    >
        ...
        <ImportTable class="custom-import" ref="importRef" @importDataTable="importDataTable" />
...
<script setup name="qualityTask">
    ...
    const data = reactive({
        ...
        relationRules: {
            ...
            'source.tableName': [{ required: true, message: '请选择源表', trigger: 'change' }],
            'source.fieldName': [
                { required: true, message: '请选择源表关联字段', trigger: 'change' }
            ],
            'target.tableName': [{ required: true, message: '请选择目标表', trigger: 'change' }],
            'target.fieldName': [
                { required: true, message: '请选择目标表关联字段', trigger: 'change' }
            ],
            relation: [{ required: true, message: '请选择关系描述', trigger: 'change' }],
    ...
    const shouldShowFormLabels = computed(() => {
        if (!relationForm.value.dataList || relationForm.value.dataList.length === 0) {
            return false;
        }
        ...
        for (const data of relationForm.value.dataList) {
            ...
            const hasIntermediateTable = data.type === 2 && data.relationMappingMiddle?.length > 0;
    ...
    const getTableName = async (datasourceId) => {
        try {
            ...
            const filteredTables = data.filter((item) => !existingTableNames.has(item.tableName));
    ...
    async function handleIntermediateTableChange(index, iIndex, tableName) {
        ...
        const tableDataItem = dbTableList.value.find((e) => e.tableName === tableName);
        //获取数据表的所有字段

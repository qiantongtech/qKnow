...
<script setup name="KnowledgeAsset">
    ...
    const governanceRows = computed(() => dashboardData.value.governance.rows || []);
    const governancePendingCount = computed(() => governanceRows.value.length);
    const governancePendingTagText = computed(() => `待处理 ${governancePendingCount.value}`);
    const showTrendChart = computed(() => dashboardData.value.mode !== 'placeholder');

            ...
            <section class="analytics-grid">
                <article class="panel-card chart-card">
                    ...
                    <div class="chart-body">
                        <div v-if="showTrendChart" ref="trendChartRef" class="trend-chart"></div>
...
<script setup name="AppOperation">
    ...
    import { Clock, InfoFilled, PieChart, TrendCharts } from '@element-plus/icons-vue';
    ...
    const industryLegendList = computed(() =>
        (dashboardData.value.industryDistribution || []).map((item, index) => ({
            ...item,
            color: industryDistributionColors[index % industryDistributionColors.length]
        }))
    );
    ...
    const paginatedLowActivityList = computed(() =>
        paginate(lowActivityList.value, lowActivityPage.value, lowActivityPageSize.value)
    );
    ...
    const showTrendChart = computed(
        () => dashboardData.value.mode !== 'placeholder' && activeTrendData.value.labels.length > 0
    );
    ...
    function renderIndustryChart() {
        ...
        industryChartInstance = createChart(industryChartRef.value, industryChartInstance);
            ...
            series: [
                {
                    ...
                    data: data.map((item, index) => ({
                        ...item,
                        itemStyle: {
                            color: industryDistributionColors[
...
<style lang="scss" scoped>
    ...
    .summary-card__icon {
        ...
        border-radius: 10px;
        box-shadow:
            0 6px 14px rgba(61, 109, 255, 0.16),

    ...
    <div class="app-container" ref="app-container">
        ...
        <div class="pagecont-bottom">
            ...
            >
                ...
                <el-table-column
                    v-if="getColumnVisibility(7) && model !== 'qa_model'"
                    label="分段内容"
                    align="left"
                    prop="content"
                >
                    <template #default="scope">
                        <div class="twoline-content">{{ scope.row.content || '-' }}</div>
                        <!--                        <div class="twoline-content">{{ scope.row.content || '-' }}</div>-->
                    </template>
                </el-table-column>
                ...
                <el-table-column
                    v-if="getColumnVisibility(9) && model === 'qa_model'"
                    label="答案"
                    align="left"
                    prop="answer"
                    show-overflow-tooltip
...
<!--                    <template #default="scope">-->
<!--                        <el-tag-->
<!--                            v-for="tag in scope.row.keywordList"-->
<!--                            v-if="scope.row.keywordList.length > 0"-->
<!--                            :key="tag"-->
<!--                            style="margin: 1px"-->
<!--                            >{{ tag || '-' }}</el-tag-->
<!--                        >-->
...
<!--                <el-table-column-->
<!--                    v-if="getColumnVisibility(12) && model !== 'hierarchical_model'"-->
<!--                    label="关键词"-->
<!--                    align="left"-->
<!--                    prop="keywords"-->
<!--                    show-overflow-tooltip-->
...
<!--                </el-table-column>-->
        ...
        >
            ...
            >
                ...
                <el-row :gutter="20">
                    <el-col :span="24">
                        <el-form-item label="分段内容" prop="content" v-if="model !== 'qa_model'">
                            <el-input
                    ...
                    <el-col :span="24">
                        <el-form-item label="问题" prop="content" v-if="model === 'qa_model'">
                            <el-input
                                v-model="form.content"
                                type="textarea"
                                placeholder="请输入内容"
...
<script setup name="KnowledgeSegment">
    ...
    // 列显隐信息
    const columns = ref([
        { key: 7, label: '分段内容', visible: true },
        { key: 9, label: '答案', visible: true },
    ...
    function getList() {
        ...
        if (proxy.$route.query.mode === 'hierarchical_model') {
            ...
            listKnowledgeSegmentTree(queryParams.value).then((response) => {
                knowledgeSegmentList.value = proxy.handleTree(response.data.rows, 'qmSegmentId');
            ...
            listKnowledgeSegment(queryParams.value).then((response) => {
                ...
                if (knowledgeSegmentList.value != null && knowledgeSegmentList.value.length > 0) {

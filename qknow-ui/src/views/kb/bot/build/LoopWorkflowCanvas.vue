        ...
        >
            ...
            <template #node-llm="nodeProps">
                <div
                    ...
                    @pointerdown.stop.prevent="onInnerNodePointerDown($event, nodeProps.id)"
                >
                    <button
                        type="button"
                    ...
                    <Handle
                        ...
                        :class="[
                            'loop-inner-handle',
                            {
                                'loop-inner-handle-drop-target': dragHoverTargetId === nodeProps.id
                            }
                        ]"
                    />
                    <Handle
                ...
                >
                    ...
                    <div class="node-body">
                        ...
                        <div class="condition-branch-list">
                            ...
                            >
                                <span class="condition-branch-name">鍒嗘敮 {{ index + 1 }}</span>
            ...
            <template #edge-default="edgeProps">
                <g class="loop-inner-edge-wrapper">
                    ...
                    <foreignObject
                        ...
                        height="24"
                    >
                        <div xmlns="http://www.w3.org/1999/xhtml" class="loop-inner-edge-add-shell">
                            <button
                                type="button"
            ...
            <Background :pattern-color="compact ? '#d7dee8' : '#dbe3ef'" :gap="compact ? 18 : 20" />
        </VueFlow>
...
<script setup>
    ...
    const emit = defineEmits(['update:workflowData', 'selectNode', 'clearSelection', 'deleteNode']);

    const flowInstanceId = `loop-workflow-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    function handleOutsidePointerDown(event) {
        ...
        const triggerElement = target?.closest?.('.loop-mini-add-btn, .loop-inner-edge-add');
    ...
    watch(
        ...
        (value) => {
            ...
            if (protectedNodes) {
                ...
                localNodes.value = applyExtentsToNodes(cloneData(protectedNodes), protectedMetrics);
    ...
    function getInnerNodeSize(node, compactMode = props.compact) {
        ...
        if (node?.type === 'reply') {
            const previewCount = Math.min(3, getLoopReplyPreviewList(node?.data).length);
            const overflowCount = getLoopReplyPreviewOverflowCount(node?.data, 3);

            return {
                width: 200,
                height: Math.max(112, 88 + previewCount * 20 + (overflowCount > 0 ? 20 : 0))
            };
        }
    ...
    function getCanvasMetricsForNodes(nodes = [], compactMode = props.compact) {
        const layout = compactMode ? LOOP_CANVAS_LAYOUTS.compact : LOOP_CANVAS_LAYOUTS.regular;
        const safeNodes = Array.isArray(nodes) && nodes.length ? nodes : [createLoopStartNode()];
    ...
    function mergeCanvasMetrics(currentMetrics, nextMetrics) {
        ...
        const layout = nextMetrics?.layout || currentMetrics.layout || layoutConfig.value;
        const layoutChanged =
        ...
        return {
            ...
            height: Math.max(currentMetrics.height || layout.minHeight, nextMetrics.height)
        };
    }
    ...
    function applyExtentsToNodes(nodes = [], metrics = workspaceMetrics.value) {
        return (nodes || []).map((node) => {
            ...
            return {
                ...cleanNode,
                position: clampNodePositionToMetrics(cleanNode.position, cleanNode, metrics)
            };
        });
    }
    ...
    function clampNodePositionToMetrics(position, nodeLike, metrics = workspaceMetrics.value) {
        ...
        if (!branchCases.length) {
            branchCases.push(
                createConditionCase(false, {
                    expression: getNodeConfigValue(data, 'expression', data.expression || '')
                })
            );
        }
    ...
    function createInnerNodeData(nodeType, index = 0, overrides = {}) {
        ...
        if (nodeType === 'reply') {
            return createStructuredNodeData({
                ...
                config: {
                    ...omitObjectKeys(overrides, ['input', 'config', 'output', 'label', 'content']),
        ...
        return createStructuredNodeData({
            ...
            config: {
                ...
                provider: normalizeModelProviderValue(
                    getNodeConfigValue(overrides, 'provider', defaultModelConfig?.provider ?? ''),
    ...
    function onInnerNodePointerDown(event, nodeId) {
        ...
        const onPointerUp = () => {
            cleanup();
            const latestNode = localNodes.value.find((item) => item.id === nodeId) || node;
            if (!hasMoved) {
            ...
            localNodes.value = applyExtentsToNodes(cloneData(committedNodes), committedMetrics);
            isDraggingInnerNode.value = false;
    ...
    function getFlowBoundsElement() {
        return canvasRef.value?.querySelector?.('.loop-workflow-stage') || canvasRef.value;
    }
    ...
    function getTargetHandlePreviewFromPointer(clientX, clientY, sourceNodeId) {
        ...
        const targetHandleElement = nodeElement.querySelector('.vue-flow__handle.target');
        const targetBounds =
            targetHandleElement?.getBoundingClientRect?.() || nodeElement.getBoundingClientRect?.();
    ...
    function showAddNodeMenuFromNode(nodeId, event, sourceHandleId = null) {
        ...
        addNodeMenuTargetId.value = null;
        addNodeMenuEdgeId.value = null;
        addNodeMenuPosition.value = getLocalMenuPosition(event.clientX, event.clientY);
    ...
    function showAddNodeMenuOnEdge(edgeProps) {
        ...
        if (canvasBounds) {
            const centerX = canvasBounds.left + (edgeProps.sourceX + edgeProps.targetX) / 2;
            const centerY = canvasBounds.top + (edgeProps.sourceY + edgeProps.targetY) / 2;
    ...
    function getInnerNodeBox(nodeLike = {}, position = nodeLike?.position || { x: 0, y: 0 }) {
    ...
    function getInnerInsertSpacingX(sourceNode = {}) {
        const isEntryNode = sourceNode.type === 'loop-start' || sourceNode.id === 'loop-entry';
    ...
    function collectInnerDownstreamNodeIds(edgeList = localEdges.value, startNodeIds = []) {
    ...
    function makeInnerInsertionRoom(
        ...
        while (attempt < 14) {
            ...
            const downstreamNodeIds = collectInnerDownstreamNodeIds(edgeList, [blockingNode.id]);
            downstreamNodeIds.add(blockingNode.id);
            nextNodes = shiftInnerNodesHorizontally(nextNodes, downstreamNodeIds, requiredShift);
            attempt += 1;
        }
    ...
    function adjustPositionToAvoidOverlap(
        ...
        let nextPosition = clampNodePositionToMetrics(position, candidateNode, metrics);
        let attempt = 0;
    ...
    function getSequentialBasePosition(sourceNode, candidateNode, sourceHandleId = null) {
        ...
        return {
            x: sourceNode.position.x + sourceSize.width + getInnerInsertSpacingX(sourceNode),
            y: alignTop
        };
    }
    ...
    function addNodeAndConnect(nodeType) {
        ...
        const sourceNode = localNodes.value.find((node) => node.id === addNodeMenuSourceId.value);
        if (!sourceNode) {
            hideAddNodeMenu();
            return;
        }
        ...
        const targetNode = isFromEdge
            ...
            : null;
        const bodyNodeCount = localNodes.value.filter((node) => node.type !== 'loop-start').length;
            ...
            isFromEdge && targetNode
                ...
                : hasExistingOutgoingTargets
                  ? getInnerSiblingBasePosition(
                    ...
                    getSequentialBasePosition(sourceNode, newNode, addNodeMenuSourceHandle.value)
                  : getSequentialBasePosition(sourceNode, newNode, addNodeMenuSourceHandle.value);
        ...
        if (isFromEdge) {
            const originalEdge = nextEdges.find((edge) => edge.id === addNodeMenuEdgeId.value);
    ...
    function onInnerConnect(connection) {
        ...
        const sourceNode = localNodes.value.find((node) => node.id === connection.source);
    ...
    function getLoopReplyPreviewList(data = {}, limit = null) {
        ...
        if (replyOutputs.length) {
            ...
            const legacyContent = `${getNodeConfigValue(data, 'content', '') || ''}`.trim();

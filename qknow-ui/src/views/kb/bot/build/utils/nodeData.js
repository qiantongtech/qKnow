...
function mergeMaps(maps = []) {
    ...
    maps.filter((item) => item instanceof Map).forEach((map) => {
...
function replaceTemplateReferenceSources(value = '', segmentMaps = []) {
    ...
    return currentValue.replace(
        ...
        (matchedValue, sourceSegment, variablePath) => {
            ...
            return nextSourceSegment ? `{{ ${nextSourceSegment}.${variablePath} }}` : matchedValue;
        }
    );
}
...
function normalizeWorkflowValue(value, context = {}, currentKey = '') {
    if (Array.isArray(value)) {
        return value.map((item) => normalizeWorkflowValue(item, context, currentKey));
    }
    ...
    const nextValue = {};
    const hasLoopGraph = Array.isArray(value.loopNodes) || Array.isArray(value.loopEdges);
...
export function normalizeWorkflowGraphIds(graph = {}, parentContext = {}) {
    ...
    const normalizedNodes = rawNodes.map((node, index) => {
        ...
        const normalizedNodeId = buildUniqueNodeId(rawNodeId || fallbackId, usedNodeIds);
...
export function createStructuredNodeData({ input = [], config = {}, output = [] } = {}) {
    return {

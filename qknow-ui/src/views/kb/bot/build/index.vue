    ...
    <div ref="appContainerRef" class="app-container workflow-editor">
        <div class="toolbar">
            ...
            <div class="toolbar-actions">
                ...
                <el-button type="primary" @click="exportFlow" :disabled="flowBuiltinFlag == 1">
        ...
        <div class="editor-container" v-loading="loading">
            <div class="flow-wrapper">
                ...
                >
                    <template #node-start="props">
                        ...
                        >
                            ...
                            <Handle
                                ...
                                class="handle"
                                :class="dragHoverTargetId === props.id ? 'handle-drop-target' : ''"
                            />
                            <Handle
                                type="source"
                    ...
                    <template #node-tool="props">
                        ...
                        >
                            ...
                            <div class="node-body">
                                ...
                                <div class="node-subtitle" v-if="getToolNodeSubtitle(props.data)">
                    ...
                    <Controls :show-zoom="true" :show-fit="true" :show-interactive="false" />
                </VueFlow>
                    ...
                    <div class="add-node-menu-list">
                        <div
                            ...
                            :key="`${menuItem.menuType}-${
                                menuItem.toolId || menuItem.name || menuItem.label || menuItem.type
                            }`"
            ...
            <div class="config-drawer" :class="{ open: drawerVisible }">
                <template v-if="drawerMode === 'node'">
                    <div
                        class="drawer-header"
                        :class="{
                            'is-start': ['start', 'llm', 'reply'].includes(selectedNode?.type)
                        }"
                    >
                    ...
                    <div class="drawer-content">
                        ...
                        <LlmNodeConfigForm
                            ...
                            :available-context-variable-groups="selectedLlmContextVariableGroups"
                            :has-upstream-context-sources="selectedLlmHasUpstreamContextSources"
                        ...
                        <ChatflowReplyNodeConfigForm
                            ...
                            :available-context-variable-groups="selectedReplyContextVariableGroups"
                            :has-upstream-context-sources="selectedReplyHasUpstreamContextSources"
                        ...
                        <ToolNodeConfigForm
                            ...
                            :tool-name="getNodeConfigValue(selectedNode?.data, 'toolName', '')"
                            :tool-source="getNodeConfigValue(selectedNode?.data, 'toolSource', '')"
                            :tool-description="
                        ...
                        <LoopNodeConfigForm
                            v-else-if="selectedNode?.type === 'loop'"
                            :item-alias="getNodeConfigValue(selectedNode?.data, 'itemAlias', '')"
                            :max-iterations="
                                Number(
...
<script setup name="ProcessFlow">
    import { ref, computed, nextTick, onActivated, onBeforeUnmount, onMounted } from 'vue';
    import {
        VueFlow,
        useVueFlow,
        Handle,
    ...
    function getDefaultLlmModelOption() {
        return (
            llmModelOptions.value
                ...
                .find((option) => `${option?.model || option?.label || ''}`.trim()) || null
        );
    }
    ...
    function normalizeToolMenuOutput(output = {}) {
        return createToolOutput({
            ...
            variableLabel: output.variableLabel || output.variableKey || output.path || output.name,
    ...
    function normalizeToolMenuItem(item = {}) {
        ...
        const toolSource = `${item?.source || item?.provider || item?.providerName || ''}`.trim();
        ...
        return {
            ...
            outputs: rawOutputs.filter(Boolean).map((output) => normalizeToolMenuOutput(output))
        };
    }
    ...
    function routerView() {
        // todo 根据类型进行判断
        if (workflowType.value == 0) {
            // 工作流
            router.push('/kb/bot/workflow?botType=0');
        } else if (workflowType.value == 1) {
            // Chatflow
        ...
        return getFlow(id)
            .then((res) => {
                ...
                if (nextNodes.length) {
                    ...
                    edges.value = Array.isArray(normalizedFlow.edges) ? normalizedFlow.edges : [];
                } else {
                    resetFlowCanvas();
                }
                // loading.value = false;
    ...
    const selectedConditionCases = computed(() =>
        selectedNode.value?.type === 'condition' ? getConditionCases(selectedNode.value.data) : []
    );
    const selectedReplyOutputs = computed(() =>
        selectedNode.value?.type === 'reply' ? getReplyOutputs(selectedNode.value.data) : []
    );
    const selectedReplyContent = computed(() =>
        selectedNode.value?.type === 'reply' ? getReplyContent(selectedNode.value.data) : ''
    );
    const selectedToolOutputs = computed(() =>
        selectedNode.value?.type === TOOL_NODE_TYPE ? getToolOutputs(selectedNode.value.data) : []
    );
    ...
    const selectedLlmContextVariableGroups = computed(() => selectedLlmContextState.value.groups);
    const selectedLlmHasUpstreamContextSources = computed(
        () => selectedLlmContextState.value.hasUpstreamContextSources
    );
    const selectedReplyContextState = computed(() => getSelectedReplyContextState());
    ...
    function validateStartFieldName(rule, value, callback) {
        ...
        if (!START_FIELD_NAME_PATTERN.test(fieldName)) {
            callback(new Error('变量名称需以字母或下划线开头，可包含数字、下划线和点号'));
            return;
        }
    ...
    function buildGeneratedId(prefix = 'node', seed = 0) {
        return normalizeTemplateReferenceSegment(`${prefix}_${Date.now()}_${seed}`, prefix);
    }
    ...
    function normalizeConditionData(data = {}) {
        ...
        if (!branchCases.length) {
            branchCases.push(createConditionCase(false, { expression: data.expression || '' }));
        }

        const elseCase = normalizedCases.find((item) => item.isElse) || createConditionCase(true);
    ...
    function getStartFieldTypeMeta(type = 'text') {
        return (
            startFieldTypeOptions.find((item) => item.value === type) || startFieldTypeOptions[0]
        );
    }
    ...
    function createStartField(overrides = {}) {
        ...
        return {
            ...
            required: overrides.required === undefined ? true : Boolean(overrides.required),
    ...
    function normalizeStartFields(fields = []) {
        ...
        return [createChatflowDefaultStartField(defaultFieldSource || {}), ...customFields];
    }
        ...
        const rawFields = structuredInput.length
            ...
            : Array.isArray(data.fields)
              ...
              : Array.isArray(data.inputFields)
                ...
                : [];
        const config = {
            ...omitObjectKeys(data, ['input', 'config', 'output', 'fields', 'inputFields']),
            ...getNodeConfig(data),
    ...
    function buildContextSourceNodeId(nodeId = '', graphPath = []) {
        ...
        const segments = [...normalizeLoopPath(graphPath), normalizedNodeId].filter(Boolean);
    ...
    function collectUpstreamContextSources(targetNodeId = '', graphContext = null) {
        ...
        const nodeList = Array.isArray(graphContext.nodeList) ? graphContext.nodeList : [];
        const edgeList = Array.isArray(graphContext.edgeList) ? graphContext.edgeList : [];
    ...
    function mergeContextSources(...sourceLists) {
        ...
        sourceLists
            ...
            .filter(Boolean)
            .forEach((source) => {
                const sourceKey = buildContextSourceNodeId(source?.node?.id, source?.graphPath);
    ...
    function getLoopEntryContextVariables(workflowData = null) {
        const itemAlias =
            `${getNodeConfigValue(workflowData, 'itemAlias', 'item') || 'item'}`.trim() || 'item';
    ...
    function getSelectedNodeContextState(allowedNodeTypes = []) {
        if (
            !selectedNode.value?.id ||
            (allowedNodeTypes.length && !allowedNodeTypes.includes(selectedNode.value?.type || ''))
        ) {
            return {
                groups: [],
        ...
        const ancestorSources =
            selectedNode.value.scope === 'loop-inner'
                ? getAncestorGraphContexts(selectedNode.value.loopPath).flatMap((graphContext) =>
    ...
    function createReplyOutput(overrides = {}) {
        const binding =
            overrides.binding && typeof overrides.binding === 'object' ? overrides.binding : {};
        ...
        return {
            ...
            sourceNodeLabel: `${overrides.sourceNodeLabel || binding.sourceNodeLabel || ''}`,
            sourceNodeType: `${overrides.sourceNodeType || binding.sourceNodeType || ''}`,
            ...
            path: `${
                overrides.path || binding.path || overrides.variableKey || binding.variableKey || ''
            }`
        };
    }
    ...
    function buildReplyPreviewVariableReferenceKey(sourceSegment = '', variablePath = '') {
        return `${sourceSegment || ''}::${variablePath || ''}`;
    }

    function getReplyPreviewVariableFallbackLabel(variablePath = '', sourceSegment = '') {
    ...
    function getReplyPreviewVariableOptionMap(nodeId = '') {
        ...
        const groups = collectUpstreamContextSources(normalizedNodeId, currentGraphContext)
            .map((source) => buildContextVariableGroup(source))
            .filter(Boolean);
    ...
    function normalizeReplyNodeData(data = {}) {
        ...
        return createStructuredNodeData({
            input: getNodeInput(data),
            config,
            output: rawOutputs.filter(Boolean).map((output) => normalizeReplyOutput(output))
        });
    }

    function getReplyContent(data = {}) {
        return `${getNodeConfigValue(normalizeReplyNodeData(data), 'content', '') || ''}`;
    }
    ...
    function getReplyNodePreviewList(data = {}, limit = null) {
        ...
        if (isChatflowWorkflow.value) {
            ...
            if (replyOutputs.length) {
                ...
                const legacyPreviewText = `${getReplyNodePreviewItems(data) || ''}`.trim();
    ...
    function createToolOutput(overrides = {}) {
        ...
        return {
            ...
            variableKey,
            variableLabel: `${overrides.variableLabel || variableKey}`.trim() || variableKey,
    ...
    function normalizeToolOutput(output = {}) {
        ...
        return {
            ...
            path: (nextOutput.path || nextOutput.variableKey).trim() || nextOutput.variableKey
        };
    }
    ...
    function normalizeToolNodeData(data = {}) {
        ...
        const config = {
            ...
            ...getNodeConfig(data),
            label: getNodeLabel(data, getNodeConfigValue(data, 'toolName', '工具')) || '工具',
            description:
            ...
            toolDescription: `${getNodeConfigValue(data, 'toolDescription', '') || ''}`.trim(),
    ...
    function getToolNodeLabel(data = {}, fallback = '工具') {
        return (
            `${getNodeLabel(data, getNodeConfigValue(data, 'toolName', fallback)) || ''}`.trim() ||
            fallback
        );
    }

    function getToolNodeSubtitle(data = {}) {
        const toolSource = `${getNodeConfigValue(data, 'toolSource', '') || ''}`.trim();
        return toolSource;
    }
    ...
    function normalizeLlmOutputItem(output = {}) {
        return {
            variableKey: `${output.variableKey || output.path || 'text'}`.trim() || 'text',
            variableLabel:
                `${output.variableLabel || output.variableKey || output.path || 'text'}`.trim() ||
                'text',
            valueType: `${output.valueType || ''}`.trim(),
    ...
    function buildLlmOutputItems(config = {}) {
        ...
        if (Boolean(config.structuredOutputEnabled) || config.responseFormat === 'json_object') {
            outputs.push({
                variableKey: 'json',
                variableLabel: 'json',
                valueType: 'object',
    ...
    function normalizeLlmNodeData(data = {}) {
        ...
        const configuredModel = `${getNodeConfigValue(data, 'model', '') || ''}`.trim();
        ...
        const config = {
            ...
            maxTokens: Math.max(
                1,
                Math.round(normalizeNumber(getNodeConfigValue(data, 'maxTokens', 4096), 4096))
            ),
            ...
            frequencyPenalty: normalizeNumber(getNodeConfigValue(data, 'frequencyPenalty', 0), 0),
            responseFormat:
            ...
            reasoningTagEnabled: Boolean(getNodeConfigValue(data, 'reasoningTagEnabled', false)),
        ...
        return createStructuredNodeData({
            input: rawMessages.filter(Boolean).map((message) => createLlmMessage(message)),
            config,
            output: structuredOutput.length
                ? structuredOutput.filter(Boolean).map((item) => normalizeLlmOutputItem(item))
    ...
    function normalizeLoopFlowNode(node, index = 0) {
        ...
        nextNode.data = normalizeLlmNodeData({
            ...
            provider: getNodeConfigValue(nextNode.data, 'provider', defaultModelConfig.provider),
                ...
                normalizeLoopFlowNode(
                    createLoopFlowNode(step.type || 'llm', index, {
                        data: createLoopFlowNodeData(step.type || 'llm', index, {
                            label:
                                step.label || `${getNodeTypeLabel(step.type || 'llm')} ${index + 1}`
                        })
                    }),
                    index
                )
    ...
    function buildLoopEdgesFromNodes(loopNodes = []) {
        ...
        for (let index = 0; index < sequence.length - 1; index += 1) {
            ...
            if (currentNode.type === 'condition') {
                connection.sourceHandle = getConditionDefaultSourceHandle(currentNode.data);
            }
    ...
    function normalizeLoopData(data = {}) {
        const legacySteps = Array.isArray(data?.steps)
            ...
            : Array.isArray(getNodeConfigValue(data, 'steps', null))
              ...
              : [];
        const rawLoopNodes = Array.isArray(getNodeConfigValue(data, 'loopNodes', null))
            ? getNodeConfigValue(data, 'loopNodes', [])
            : Array.isArray(data.loopNodes)
              ? data.loopNodes
              : [];
        const rawLoopEdges = Array.isArray(getNodeConfigValue(data, 'loopEdges', null))
            ? getNodeConfigValue(data, 'loopEdges', [])
            : Array.isArray(data.loopEdges)
              ? data.loopEdges
              : [];
        let nextLoopNodes = Array.isArray(rawLoopNodes) ? cloneNodeData(rawLoopNodes) : [];
        let nextLoopEdges = Array.isArray(rawLoopEdges) ? cloneNodeData(rawLoopEdges) : [];

        if (!nextLoopNodes.length && Array.isArray(legacySteps) && legacySteps.length) {
        ...
        const normalizedLoopNodes = [normalizeLoopFlowNode(entryNode, 0), ...bodyNodes];
        const loopNodeIds = new Set(normalizedLoopNodes.map((node) => node.id));
        const normalizedLoopEdges = nextLoopEdges
            .filter((edge) => edge && loopNodeIds.has(edge.source) && loopNodeIds.has(edge.target))
    ...
    function getLoopCanvasMetrics(data = {}, compactMode = true) {
        ...
        return {
            ...
            height: Math.max(layout.minHeight, Math.ceil(maxBottom + layout.paddingBottom))
        };
    }
    ...
    function normalizeLoopExternalEdges(nodeIds = []) {
        ...
        const nextEdges = edges.value.map((edge) => {
            ...
            if (loopNodeIds.has(edge.source) && edge.sourceHandle !== LOOP_OUTER_SOURCE_HANDLE_ID) {
                nextEdge = {
                    ...nextEdge,
            ...
            if (loopNodeIds.has(edge.target) && edge.targetHandle !== LOOP_OUTER_TARGET_HANDLE_ID) {
                nextEdge = {
                    ...nextEdge,
    ...
    function updateInnerNodeDataAtPath(loopPath = [], nodeId, nodeType, updater) {
        ...
        updateLoopWorkflowDataAtPath(loopPath, (currentLoopData) => {
            ...
            const nextLoopNodes = getLoopNodes(normalizedLoopData).map((node) => {
                ...
                changed = true;
                const currentNodeData = normalizeNodeData(node.type, cloneNodeData(node.data));
                const nextRawData =
                    typeof updater === 'function'
    ...
    function removeInnerNodeAtPath(loopPath = [], nodeId) {
        ...
        updateLoopWorkflowDataAtPath(loopPath, (currentLoopData) => {
            ...
            return mergeNodeConfig(normalizedLoopData, {
                loopNodes: getLoopNodes(normalizedLoopData).filter((node) => node.id !== nodeId),
    ...
    function updateLoopEdgesAtPath(loopPath = [], updater) {
        updateLoopWorkflowDataAtPath(loopPath, (currentLoopData) => {
            ...
            const nextEdges =
                typeof updater === 'function'
                    ? updater(cloneNodeData(getLoopEdges(normalizedLoopData)), normalizedLoopData)
                    : updater;
    ...
    function handleInnerNodeSelect(payload = {}) {
        ...
        hideAddNodeMenu();
        debugRunPanelVisible.value = false;
        const nextData = normalizeNodeData(payload.type, cloneNodeData(payload.data || {}));
    ...
    function updateNodeData(nodeId, updater) {
        ...
        const currentData = normalizeNodeData(targetNode.type, cloneNodeData(targetNode.data));
        const nextRawData =
            typeof updater === 'function' ? updater(cloneNodeData(currentData)) : updater;
        const nextData = normalizeNodeData(
            targetNode.type,
    ...
    function normalizeExternalConnection(connection = {}, nodeList = nodes.value) {
        ...
        const sourceNode = normalizedNodeList.find((node) => node.id === connection.source);
        const targetNode = normalizedNodeList.find((node) => node.id === connection.target);

        return {
            ...connection,
            sourceHandle:
                connection.sourceHandle || getDefaultSourceHandleId(sourceNode) || undefined,
            targetHandle:
                connection.targetHandle || getDefaultTargetHandleId(targetNode) || undefined
        };
    }
    ...
    function getOuterNodeApproxSize(nodeLike = {}) {
        ...
        if (nodeType === 'reply') {
            ...
            const previewCount = Math.min(3, getReplyNodePreviewList(nodeLike?.data).length);
            const overflowCount = getReplyNodePreviewOverflowCount(nodeLike?.data, 3);

            return {
                width: 220,
                height: Math.max(122, 96 + previewCount * 20 + (overflowCount > 0 ? 20 : 0))
            };
        }

        if (nodeType === TOOL_NODE_TYPE) {
            const previewCount = Math.min(3, getToolNodePreviewList(nodeLike?.data).length);
            ...
            return {
                ...
                height: Math.max(
                    122,
                    96 + (hasSubtitle ? 26 : 0) + previewCount * 20 + (overflowCount > 0 ? 20 : 0)
                )
            };
        }
    ...
    function getOuterNodeBox(nodeLike = {}, position = nodeLike?.position || { x: 0, y: 0 }) {
    ...
    function getOuterSequentialBasePosition(sourceNode, candidateNode, sourceHandleId = null) {
        ...
        return {
            x: sourceNode.position.x + sourceSize.width + getOuterInsertSpacingX(sourceNode),
            y: alignTop
        };
    }
    ...
    function getOuterEdgeBasePosition(sourceNode, targetNode, candidateNode) {
        ...
        const centeredX = Math.round((sourceBox.right + targetBox.left - candidateSize.width) / 2);
        ...
        return {
            ...
            y: Math.round((sourceCenterY + targetCenterY) / 2 - candidateSize.height / 2)
        };
    }
    ...
    function collectDownstreamNodeIds(edgeList = edges.value, startNodeIds = []) {
        ...
        while (queue.length) {
            ...
            edgeList.forEach((edge) => {
                if (edge.source === currentId && edge.target && !visited.has(edge.target)) {
    ...
    function shiftOuterNodesHorizontally(nodeList = nodes.value, nodeIds = new Set(), deltaX = 0) {
        if (!deltaX) {
    ...
    function makeOuterInsertionRoom(
        ...
        if (options.targetNodeId) {
            const targetNode = nextNodes.find((node) => node.id === options.targetNodeId);
            if (targetNode) {
                const targetBox = getOuterNodeBox(targetNode);
                const requiredShift = Math.ceil(candidateBox().right + 40 - targetBox.left);

                if (requiredShift > 0) {
                    const downstreamNodeIds = collectDownstreamNodeIds(edgeList, [targetNode.id]);
        ...
        while (attempt < 14) {
            ...
            const downstreamNodeIds = collectDownstreamNodeIds(edgeList, [blockingNode.id]);
            downstreamNodeIds.add(blockingNode.id);
            nextNodes = shiftOuterNodesHorizontally(nextNodes, downstreamNodeIds, requiredShift);
            attempt += 1;
        }
    ...
    function adjustPositionToAvoidOverlap(
        ...
        while (attemptCount < maxAttempts) {
            ...
            const overlappingNode = nodeList.find((node) => {
                ...
                return isOuterBoxOverlapping(candidateBox, getOuterNodeBox(node), minGapX, minGapY);
            });
    ...
    function removeLlmMessage(messageId) {
        ...
        selectedNode.value.data = setNodeInput(
            ...
            getNodeInput(selectedNode.value.data).filter((message) => message.id !== messageId)
        );
        syncSelectedNodeData();
    }
    ...
    async function saveStartField() {
        ...
        if (startFieldDialogMode.value === 'edit' && isReadonlyStartField(startFieldDraft.value)) {
            return;
        }
    ...
    function removeStartField(fieldId) {
        ...
        selectedNode.value.data = setNodeInput(
            selectedNode.value.data,
            getStartFields(selectedNode.value.data).filter((field) => field.id !== fieldId)
        );
        syncSelectedNodeData();
    }
    ...
    function addConditionCase() {
        ...
        const elseCase = cases.find((item) => item.isElse) || createConditionCase(true);
        const branchCases = cases.filter((item) => !item.isElse);

        selectedNode.value.data = setNodeConfigValue(selectedNode.value.data, 'cases', [
            ...branchCases,
    ...
    function removeConditionCase(caseId) {
        ...
        if (selectedEntry.scope === 'loop-inner') {
            updateLoopEdgesAtPath(selectedEntry.loopPath, (currentEdges) =>
                currentEdges.filter(
                    (edge) => !(edge.source === selectedEntry.id && edge.sourceHandle === handleId)
                )
            );
            return;
        }
        ...
        const relatedEdges = edges.value
            .filter((edge) => edge.source === selectedEntry.id && edge.sourceHandle === handleId)
            .map((edge) => edge.id);
    ...
    function onConnect(connection) {
        ...
        const duplicatedEdge = edges.value.find(
            (edge) =>
                ...
                (edge.sourceHandle || null) === (normalizedConnection.sourceHandle || null) &&
                (edge.targetHandle || null) === (normalizedConnection.targetHandle || null)
        );
    ...
    function getTargetHandlePreviewFromPointer(clientX, clientY, sourceNodeId) {
        ...
        const targetHandleId = targetHandleElement?.getAttribute?.('data-handleid') || undefined;
    ...
    function onSourceActionPointerDown(event, nodeId, sourceHandleId = null) {
        ...
        const onPointerMove = (moveEvent) => {
            const currentPosition = getPointerPositionInFlow(moveEvent.clientX, moveEvent.clientY);
            if (!currentPosition) {
    ...
    function shouldIgnoreAddMenuDismiss() {
        return addNodeMenuVisible.value && Date.now() - addMenuOpenAt.value < addMenuDismissGuardMs;
    }
    ...
    function showAddNodeMenu(nodeId, event, sourceHandleId = null) {
        ...
        addNodeMenuPosition.value = getAddNodeMenuViewportPosition(event.clientX, event.clientY);
    ...
    function getAddNodeMenuViewportPosition(x, y) {
        ...
        return {
            ...
            y: Math.min(Math.max(ADD_NODE_MENU_MARGIN, y - ADD_NODE_MENU_VERTICAL_OFFSET), maxY)
        };
    }
    ...
    function addNodeAndConnect(nodeType, nodeDataOverride = null) {
        ...
        const isFromEdge = Boolean(addNodeMenuEdgeId.value && addNodeMenuTargetId.value);
        const sourceNode = nodes.value.find((node) => node.id === addNodeMenuSourceId.value);
        ...
        const newNodeDefaultSourceHandle = getDefaultSourceHandleId(candidateNode) || undefined;
        const newNodeDefaultTargetHandle = getDefaultTargetHandleId(candidateNode) || undefined;
                ...
                createNormalizedExternalEdge(
                    {
                        ...
                        sourceHandle:
                            originalEdge.sourceHandle || addNodeMenuSourceHandle.value || undefined,
                        target: newNodeId,
    ...
    function showAddNodeMenuOnEdge(edgeProps) {
        ...
        addNodeMenuPosition.value = getAddNodeMenuViewportPosition(centerX + 20, centerY);
    ...
    function handleSelectedNodeFieldUpdate(payload = {}) {
        ...
        selectedNode.value.data = setNodeConfigValue(selectedNode.value.data, field, value);
        syncSelectedNodeData();
    }
    ...
    function removeToolOutput(outputId) {
        ...
        selectedNode.value.data = setNodeOutput(
            selectedNode.value.data,
            getToolOutputs(selectedNode.value.data).filter((output) => output.id !== outputId)
        );
        syncSelectedNodeData();
    }
    ...
    function removeReplyOutput(outputId) {
        ...
        selectedNode.value.data = setNodeOutput(
            selectedNode.value.data,
            getReplyOutputs(selectedNode.value.data).filter((output) => output.id !== outputId)
        );
        syncSelectedNodeData();
    }
    ...
    function validateFlow(options = {}) {
        ...
        if (invalidReplyNode) {
            ElMessage.warning(
                isChatflowWorkflow.value
                    ? `回复节点“${getNodeLabel(invalidReplyNode.data, '回复')}”请填写回复内容`
                    : `输出节点“${getNodeLabel(

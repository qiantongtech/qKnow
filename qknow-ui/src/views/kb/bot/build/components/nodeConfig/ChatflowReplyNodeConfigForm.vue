...
<script setup>
    ...
    function createEditorVariableChip(segment = {}) {
        ...
        tag.className = 'el-tag el-tag--light el-tag--small chatflow-reply-context-chip-tag';
    ...
    function restoreSelection() {
        ...
        const startPosition = locateEditorPosition(editorElement, editorSelection.value.start);
        const endPosition = locateEditorPosition(editorElement, editorSelection.value.end);
    ...
    function shouldOpenSlashVariableMenu() {
        ...
        const content = getEditorContent();
        const caret = Math.max(0, Math.min(editorSelection.value.start, content.length));

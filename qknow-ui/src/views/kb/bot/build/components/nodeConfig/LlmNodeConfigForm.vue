            ...
            <div class="llm-message-card">
                <div class="llm-message-editor-shell">
                    ...
                    <div
                        ...
                        :class="{
                            'is-empty': !node.data.prompt && !isEditorComposing({ type: 'prompt' })
                        }"
            ...
            >
                                    ...
                                    <template v-if="getContextVariableGroups().length">
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                @mousedown.prevent
                                                @click="
                                                    insertVariableIntoMessage(message.id, option)
                                                "
                                            >
                                                <el-tag
                                                    disable-transitions
                ...
                <div class="llm-message-editor-shell">
                    <div
                        ...
                        "
                        @click="handleEditorSelectionChange({ type: 'message', id: message.id })"
                        @keyup="handleEditorSelectionChange({ type: 'message', id: message.id })"
                        @mouseup="handleEditorSelectionChange({ type: 'message', id: message.id })"
                        @beforeinput="
                            handleEditorBeforeInput({ type: 'message', id: message.id }, $event)
                        "
                        @input="handleEditorInput({ type: 'message', id: message.id }, $event)"
                        @keydown="handleEditorKeydown({ type: 'message', id: message.id }, $event)"
                        @paste="handleEditorPaste({ type: 'message', id: message.id }, $event)"
                    ></div>
                </div>
            </div>
        </div>
...
<script setup>
    ...
    const emit = defineEmits(['updateField', 'updateMessage', 'addMessage', 'removeMessage']);
    ...
    function findModelOptionByBindingValue(bindingValue = '') {
        return (
            getFlatModelOptions().find(
                (option) => `${option?.value || ''}`.trim() === `${bindingValue || ''}`.trim()
            ) || null
        );
    }
    ...
    function findModelOptionByModelName(modelName = '') {
        return (
            getFlatModelOptions().find(
                (option) =>
                    `${option?.model || option?.label || ''}`.trim() === `${modelName || ''}`.trim()
            ) || null
        );
    }
    ...
    function getMessagePlaceholder(role) {
        return role === 'assistant' ? assistantMessagePlaceholder : userMessagePlaceholder;
    }
    ...
    function handleMessageVariablePopoverVisibleChange(messageId = '', visible = false) {
        if (!messageId) {
            return;
        }
    ...
    function getEditorContent(target = {}) {
        if (target.type === 'message') {
            return (
                (props.node?.data?.messages || []).find((message) => message.id === target.id)
                    ?.content || ''
            );
        }
    ...
    function renderEditorContent(target = {}, { preserveSelection = true, force = false } = {}) {
        const editorElement = getEditorElement(target);
        ...
        function walk(currentNode, accumulated) {
            if (currentNode === container) {
                if (currentNode.nodeType === 3) {
                    return {
                        found: true,
                        offset: accumulated + Math.min(offset, currentNode.textContent?.length || 0)
                    };
                }
    ...
    function saveEditorSelection(target = {}, options = {}) {
        ...
        editorSelections.value[targetKey] = {
            start: getSelectionOffset(editorElement, range.startContainer, range.startOffset),
    ...
    function restoreEditorSelection(target = {}) {
        ...
        const startPosition = locateEditorPosition(editorElement, selectionOffsets.start);
    ...
    function insertTextAtSelection(content = '', insertedText = '', selection = {}) {
        ...
        const end = Number.isInteger(selection.end)
            ...
            : start;
        const nextValue = currentContent.slice(0, start) + insertedText + currentContent.slice(end);
    ...
    function removeTextAtSelection(content = '', selection = {}, direction = 'backward') {
        const currentContent = `${content || ''}`;
        ...
        if (start !== end) {
            ...
            const adjacentRange = ranges.find((range) => start > range.start && start <= range.end);
    ...
    function buildTemplateVariableReferenceKey(sourceSegment = '', variablePath = '') {
        return `${sourceSegment || ''}::${variablePath || ''}`;
    }
    ...
    function getTemplateVariableOptionMap() {
        ...
        getContextVariableGroups().forEach((group) => {
            (group.variables || []).forEach((option) => {
                ...
                const sourceSegment = buildTemplateReferenceSegment(
                    normalizedBinding.sourceNodeId || normalizedBinding.sourceNodeType || 'node',
                    'node'
                );
                const variablePath = `${
    ...
    function syncSlashVariableMenu(target = {}) {
        if (shouldOpenSlashVariableMenu(target)) {
            ...
            if (target.type === 'message') {
                closePromptVariablePopover();
                messageVariablePopoverVisibleMap.value = target.id ? { [target.id]: true } : {};
            } else {
                closeAllMessageVariablePopovers();

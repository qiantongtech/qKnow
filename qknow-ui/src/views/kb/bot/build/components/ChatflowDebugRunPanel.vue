...
<template>
    <div class="chatflow-debug-run-panel">
        <div v-if="showSections" class="chatflow-debug-run-panel__input-block">
            ...
            <el-form class="chatflow-debug-run-panel__form" @submit.prevent label-width="100px">
                <el-row :gutter="20">
                    <el-col :span="24">
        ...
        <form class="chatflow-debug-run-panel__composer" @submit.prevent="handleSend">
            <el-input
                v-model="prompt"
...
<script setup>
    ...
    const canSend = computed(() => !running.value && Boolean(`${prompt.value || ''}`.trim()));
    ...
    function detectChatPromptField(fields = []) {
        ...
        return (
            ...
            normalizedFields.find((field) => {
                ...
                return fieldKey === 'user' || fieldLabel === 'userinput' || fieldLabel === '用户';
            }) ||
            normalizedFields.find((field) => {
                const fieldKey = getFieldKey(field).toLowerCase();
                const fieldLabel = getFieldLabel(field).toLowerCase();
    ...
    function buildBaseInputValues() {
        return (Array.isArray(props.fields) ? props.fields : []).reduce((result, field) => {
            const fieldKey = getFieldKey(field);
    ...
    function normalizeDebugError(error) {
        ...
        return `${error?.message || ''}`.trim() || 'Chatflow 调试请求失败，请检查接口接入实现。';
    }
    ...
    function parsePlainStreamText(
        ...
        if (nextText.startsWith(currentContent) || nextText.length >= currentContent.length) {
    ...
    function parseStreamChunk(rawData = '', currentContent = '', hasStructuredTextChunk = false) {
        ...
        if (Number(outer?.code ?? 200) !== 200) {
            throw new Error(`${outer?.msg || outer?.message || 'Chatflow 调试请求失败'}`.trim());
        }
        ...
        try {
            ...
            if (typeof inner === 'string') {
                return parsePlainStreamText(inner, currentContent, hasStructuredTextChunk);
            }

            return createStreamChunkResult();
        } catch {
            return parsePlainStreamText(rawInner, currentContent, hasStructuredTextChunk);
        }
    }
    ...
    function stopStream(markAsManual = true, controller = conversationInAbortController.value) {

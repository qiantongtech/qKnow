...
<template>
    <div class="app-container" ref="app-container">
                        ...
                        <el-col :span="24">
                            ...
                            <el-form-item label="知识库">
                                <el-button
                                    ...
                                    plain
                                >
                                    <i class="iconfont-mini icon-upload-cloud-line mr5"></i
                                    >导入知识库</el-button
                                >
                            </el-form-item>
                            <div style="margin: 0 0 10px 80px">
                                    ...
                                    <el-table-column label="操作" width="80" align="center">
                                        <template #default="{ $index }">
                                            <el-button
                                                type="danger"
                                                size="small"
                                                @click="handleDeleteKnowledge($index)"
                                                >删除</el-button
                                            >
                                        </template>
                                    </el-table-column>
        ...
        <!-- 知识库选择组件 -->
        <knowledge-base-multiple ref="knowledgeBaseMultipleRef" @confirm="handleKnowledgeConfirm" />
...
<script setup name="Agent">
    ...
    // 表单验证规则
    const formRules = {
        modelId: [{ required: true, message: '请选择大模型', trigger: 'change' }]
    };
    ...
    function getModelList() {
        getChatModelDict().then((res) => {
            ...
            if (
                ...
                modelOptions.value[0].models?.length > 0
            ) {
    ...
    function addItem() {
        ...
        const newId =
            form.variables.length > 0 ? Math.max(...form.variables.map((v) => v.id)) + 1 : 1;

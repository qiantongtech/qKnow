...
<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="工具数量" align="center" prop="toolNum">
        <template #default="scope">
          {{ scope.row.toolNum || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="状态" align="center" prop="status">
        <template #default="scope">
          <div>
            <dict-tag :options="bot_mcp_status" :value="scope.row.status"/>
          </div>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="McpMultiple">
import {listMcpConfig} from "@/api/kb/mcp/mcpConfig";
import {ref} from "vue";
...
function getList() {
  loading.value = true;
  queryParams.value.status = 1; // 只查询已启动的mcp
  listMcpConfig(proxy.addDateRange(queryParams.value, dateRange.value)).then(
      async (response) => {

...
<script setup name="kbToolCategory">
import {
  getToolCategory,
  delToolCategory,
  addToolCategory,
...
function handleUpdate(row) {
  ...
  getToolCategory(_id).then((response) => {
    ...
    if (res.data.total !== 0) {
      ...
      getAllList({parentId: row.id}).then((res) => {
        if (res.data.length !== 0) {
          ...
          // 提示确认删除
          proxy.$modal
              .confirm('是否确认删除工具分类名称为"' + name + '"的数据项？')
              .then(function () {
                return delToolCategory(_ids);
              })
              .then(() => {
                getList();

      ...
      <div class="steps-inner">
        <ul class="zl-step">
          ...
          >
            <div
              class="step-circle"
              :class="{ active: activeStep === index, prev: index < activeStep }"
            >
              <span>{{ index + 1 }}</span>
            </div>
    ...
    <div class="page-content">
      <div class="content-main">
        <div v-show="activeStep === 0" class="connection-panel">
          <StorageConnectionStep ref="connectionStepRef" @change="handleConnectionChange" />
        </div>
        ...
        <StorageFileSelectStep
          ...
          :resolve-candidates-api="resolveStorageCandidates"
          :load-tree-api="getStorageDirectoryTree"
          :list-files-api="listStorageFiles"
          :preview-file-api="prepareStorageFilePreview"
          :download-file-api="downloadStorageFile"
          :supported-extensions="KG_SUPPORTED_EXTENSIONS"
        />

        <div v-show="activeStep === 2" class="document-panel">
          <div class="section-title">文件信息</div>
          <KgStorageDocumentStep ref="documentStepRef" />
        </div>
      </div>
...
<script setup name="KgKnowledgeDocumentStorageSync">
...
import KgStorageDocumentStep from '@/components/ThirdStorage/KgStorageDocumentStep.vue';
import {
  downloadStorageFile,
...
} from '@/api/kg/knowledge/storageSync';
...
function closeAndReturnToList() {
  return proxy.$tab
    .closeOpenPage({ path: '/kg/knowledge/document' })
...
async function goNext() {
  ...
  if (
    ...
    testedConnectionFingerprint.value !== getConnectionFingerprint(currentConnection)
  ) {
...
async function handleTestConnection() {
  ...
  try {
    const response = await testStorageConnection({ connection: connectionData.value });
    const currentConnection = connectionStepRef.value?.getFormData() || {};
...
async function handleConfirm() {
  ...
  try {
    ...
    ElMessage.success(response?.msg || `成功导入 ${candidateFiles.value.length} 个文件`);

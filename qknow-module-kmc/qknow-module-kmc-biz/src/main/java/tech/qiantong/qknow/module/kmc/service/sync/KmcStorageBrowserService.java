...
import tech.qiantong.qknow.module.kmc.controller.admin.sync.vo.KmcStorageFileRespVO;
...
@Service
public class KmcStorageBrowserService {
    ...
    private static final HashSet<String> JSON_FILE_EXTENSIONS =
            new HashSet<>(Arrays.asList("json", "jsonl"));
    private static final HashSet<String> JSON_STYLES =
            new HashSet<>(Arrays.asList("Alpaca", "ShareGPT", "Multilingual Thinking"));
    ...
    public void testConnection(KmcStorageBrowseReqVO.Connection connection) {
    ...
    public List<KmcStorageFileRespVO> listDirectories(KmcStorageBrowseReqVO request) {
    ...
    public Map<String, Object> listFiles(KmcStorageBrowseReqVO request) {
        List<KmcStorageFileRespVO> entries = listEntries(
    ...
    public Map<String, Object> resolveCandidates(KmcStorageSelectionReqVO request) {
        ...
        result.put("hasSemiStructured", candidates.accepted().stream()
                .anyMatch(item -> isJsonFile(item.getName())));
        return result;
    }

    @Transactional(rollbackFor = Exception.class)
    public int importDocuments(
            KmcStorageImportReqVO request,
            Long creatorId,
            String creatorName,
            Long workspaceId) {
        CandidateFiles candidates = collectCandidates(request);
        ...
        boolean hasJson = candidates.accepted().stream().anyMatch(item -> isJsonFile(item.getName()));
        if (hasJson && !JSON_STYLES.contains(request.getJsonStyle())) {
            throw new ServiceException("所选文件包含半结构化文件，请选择有效的数据风格");
        }
        ...
        try {
            ...
            List<LocalImportFile> textFiles = new ArrayList<>();
            List<LocalImportFile> jsonFiles = new ArrayList<>();
            for (KmcStorageFileRespVO remoteFile : candidates.accepted()) {
                ...
                LocalImportFile localFile = new LocalImportFile(originalName, relativePath);
                (isJsonFile(originalName) ? jsonFiles : textFiles).add(localFile);
            }

            int importedCount = 0;
            if (!textFiles.isEmpty()) {
                importedCount += kmcDocumentService.createKmcDocument(buildDocumentRequest(
                        request, textFiles, "text", null, creatorId, creatorName, workspaceId));
            }
            if (!jsonFiles.isEmpty()) {
                importedCount += kmcDocumentService.createKmcDocument(buildDocumentRequest(
                        request, jsonFiles, "json", request.getJsonStyle(), creatorId, creatorName, workspaceId));
            }
    ...
    private CandidateFiles collectCandidates(KmcStorageSelectionReqVO request) {
        LinkedHashMap<String, KmcStorageFileRespVO> accepted = new LinkedHashMap<>();
        LinkedHashMap<String, KmcStorageFileRespVO> rejected = new LinkedHashMap<>();
        for (KmcStorageSelectionReqVO.SelectedItem item : request.getSelectedItems()) {
            ...
            if (Boolean.TRUE.equals(item.getDirectory())) {
                ...
                String actualName = getName(path);
                addCandidate(KmcStorageFileRespVO.builder()
                        .name(StrUtil.blankToDefault(actualName, item.getName()))
                        .path(path)
                        .directory(false)
                        .hasChildren(false)
    ...
    private void collectDirectoryCandidates(
            KmcStorageBrowseReqVO.Connection connection,
            String directory,
            Map<String, KmcStorageFileRespVO> accepted,
            Map<String, KmcStorageFileRespVO> rejected) {
        for (KmcStorageFileRespVO item : listEntries(connection, directory)) {
            if (Boolean.TRUE.equals(item.getDirectory())) {
    ...
    private KmcDocumentSaveReqVO buildDocumentRequest(
            KmcStorageImportReqVO source,
            ...
            String creatorName,
            Long workspaceId) {
        KmcDocumentSaveReqVO target = new KmcDocumentSaveReqVO();
        target.setKnowledgeBaseId(source.getKnowledgeBaseId());
        target.setKnowledgeBaseName(source.getKnowledgeBaseName());
        ...
        target.setJsonStyle(jsonStyle);
        target.setMode("custom");
        target.setParentMode("paragraph");
        target.setRemoveExtraSpaces(true);
        target.setRemoveUrlsEmails(false);
        target.setChunkOverlap("50");
        target.setMaxTokens(1024);
        target.setSeparator("\n\n");
        target.setDocForm("json".equals(fileType) ? "qa_model" : "text_model");
        target.setDocLanguage("Chinese");
        target.setSubchunkMaxTokens(512);
        target.setSubchunkSeparator("\n");
    ...
    private boolean isSupportedFile(String name) {
        ...
        return TEXT_FILE_EXTENSIONS.contains(extension) || JSON_FILE_EXTENSIONS.contains(extension);
    }

    private boolean isJsonFile(String name) {
        return JSON_FILE_EXTENSIONS.contains(getFileExtension(name));
    }
    ...
    private record CandidateFiles(
            List<KmcStorageFileRespVO> accepted,
            List<KmcStorageFileRespVO> rejected) {
    }

    private record LocalImportFile(String name, String path) {
    }

    public String preparePreview(KmcStorageBrowseReqVO request) {
    ...
    public void download(KmcStorageBrowseReqVO request, HttpServletResponse response) {
        String path = normalizePath(request.getPath());
        try {
    ...
    private boolean isDirectory(KmcStorageBrowseReqVO.Connection connection, String path) throws Exception {
        String type = normalizeType(connection.getDatasourceType());
        return switch (type) {
            case "HDFS" -> {
    ...
    private void writeFile(
            KmcStorageBrowseReqVO.Connection connection, String path, OutputStream output) throws Exception {
        String type = normalizeType(connection.getDatasourceType());
        switch (type) {
            case "HDFS" -> {
                try (FileSystem fileSystem = createHdfsFileSystem(connection);
    ...
    private List<KmcStorageFileRespVO> listEntries(
            KmcStorageBrowseReqVO.Connection connection, String path) {
        String type = normalizeType(connection.getDatasourceType());
        try {
            List<KmcStorageFileRespVO> entries = switch (type) {
            ...
            entries.sort(Comparator
                    .comparing(KmcStorageFileRespVO::getDirectory, Comparator.reverseOrder())
                    .thenComparing(KmcStorageFileRespVO::getName, String.CASE_INSENSITIVE_ORDER));
            return entries;
        } catch (ServiceException exception) {
            throw exception;
        } catch (Exception exception) {
    ...
    private void testHdfs(KmcStorageBrowseReqVO.Connection connection) throws Exception {
        try (FileSystem fileSystem = createHdfsFileSystem(connection)) {
    ...
    private List<KmcStorageFileRespVO> listHdfs(
            KmcStorageBrowseReqVO.Connection connection, String path) throws Exception {
        List<KmcStorageFileRespVO> result = new ArrayList<>();
        try (FileSystem fileSystem = createHdfsFileSystem(connection)) {
            FileStatus[] statuses = fileSystem.listStatus(new Path(path));
            for (FileStatus status : statuses) {
                boolean directory = status.isDirectory();
                result.add(KmcStorageFileRespVO.builder()
    ...
    private FileSystem createHdfsFileSystem(KmcStorageBrowseReqVO.Connection connection) throws Exception {
    ...
    private void testFtp(KmcStorageBrowseReqVO.Connection connection) throws Exception {
        FTPClient client = connectFtp(connection);
        disconnectFtp(client);
    }

    private List<KmcStorageFileRespVO> listFtp(
            KmcStorageBrowseReqVO.Connection connection, String path) throws Exception {
        FTPClient client = connectFtp(connection);
        try {
            FTPFile[] files = client.listFiles(path);
    ...
    private FTPClient connectFtp(KmcStorageBrowseReqVO.Connection connection) throws Exception {
    ...
    private void testOss(KmcStorageBrowseReqVO.Connection connection) {
        OSS oss = createOssClient(connection);
        try {
            String bucket = requireValue(connection.getBucket(), "OSS Bucket");
    ...
    private List<KmcStorageFileRespVO> listOss(
            KmcStorageBrowseReqVO.Connection connection, String path) {
        OSS oss = createOssClient(connection);
        try {
            String bucket = requireValue(connection.getBucket(), "OSS Bucket");
    ...
    private OSS createOssClient(KmcStorageBrowseReqVO.Connection connection) {

 ...
 */
@Data
public class ShareGPTBO implements JsonStyle {

    List<ShareGPTMessageBO> messages;
    ...
    public KmcDocumentSegmentDO toKmcDocumentSegmentDO(int position) {
        ...
        for (ShareGPTMessageBO message : this.getMessages()) {
            if (message.getRole().equals("user")) {
                result.setContent(message.getContent());
                result.setSignContent(message.getContent());
            } else if (message.getRole().equals("assistant")) {
                result.setAnswer(message.getContent());
            }
        }
